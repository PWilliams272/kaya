...
The frontend (`app.js`/`app.css`/`tokens.css`/`index.html`) has no framework lock-in — it only depends on relative `/api/...` and `/viewer-data/...` fetch endpoints, not on FastAPI-specific templating, so it can be dropped into any backend's static folder largely unmodified. `tokens.css` in particular is a good candidate to extract as a shared design-token file across the whole site if consistent theming across components is a goal.
